...
function limit(userLowerLimit, userUpperLimit) {
    ...
    fetch('https://pokeapi.co/api/v2/pokemon/?offset=' + userLowerLimit + '&limit=' + (userUpperLimit-userLowerLimit))
    .then(
    function(response) {
    ...
    response.json().then(function(data) {
        ...
        tbody;
        console.log(tbody);
        let string = '<tr><td>'+data.results[i].name+'</td>';
    //    <td>'+data.results[i].url+'</tr>'
